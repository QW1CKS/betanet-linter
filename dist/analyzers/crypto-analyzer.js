...
class CryptoAnalyzer {
    async analyze(binaryPath) {
        ...
        const hasChaCha20 = ciphers.some(c => c.toLowerCase().includes('chacha20') ||
            c.toLowerCase().includes('chacha'));
        const hasEd25519 = signatures.some(s => s.toLowerCase().includes('ed25519') ||
            s.toLowerCase().includes('eddsa'));
        const hasX25519 = keyExchange.some(k => k.toLowerCase().includes('x25519') ||
    ...
    async detectCiphers(binaryPath) {
        try {
    ...
    async detectSignatures(binaryPath) {
        try {
            const { stdout } = await (0, execa_1.default)('strings', [binaryPath]);
            const signatures = [];
    ...
    async detectKeyExchange(binaryPath) {
        try {
            const { stdout } = await (0, execa_1.default)('strings', [binaryPath]);
            const keyExchange = [];
    ...
    async detectCryptoLibraries(binaryPath) {
        try {
            ...
            const { stdout: stringsOut } = await (0, execa_1.default)('strings', [binaryPath]);
            const libPatterns = [
                /OpenSSL/i,
                /BoringSSL/i,
                /LibreSSL/i,
    ...
    async hasBetanetCryptoSuite(binaryPath) {
        const analysis = await this.analyze(binaryPath);
        // Check for required Betanet crypto primitives
        const hasSHA256 = analysis.hashes.some(h => h.toLowerCase().includes('sha256'));
        const hasChaCha20Poly1305 = analysis.ciphers.some(c => c.toLowerCase().includes('chacha20') &&
    ...
    async hasPostQuantumSupport(binaryPath) {
        const analysis = await this.analyze(binaryPath);
        return analysis.postQuantum || analysis.hasKyber;
    }
    async getCryptoComplianceScore(binaryPath) {
        const analysis = await this.analyze(binaryPath);
        let score = 0;
        const maxScore = 100;
        // Required algorithms (40 points)
        if (analysis.hashes.some(h => h.toLowerCase().includes('sha256')))
            score += 10;
        if (analysis.hasChaCha20)
            score += 15;
        if (analysis.hasEd25519)
            score += 10;
        if (analysis.hasX25519)
            score += 5;
        // Post-quantum support (30 points)
        if (analysis.hasKyber)
            score += 30;
        // Additional features (30 points)
        if (analysis.libraries.length > 0)
            score += 10;
        if (analysis.ciphers.length > 3)
            score += 10;
        if (analysis.signatures.length > 2)
            score += 10;
        return Math.min(score, maxScore);
    }
}
exports.CryptoAnalyzer = CryptoAnalyzer;
//# sourceMappingURL=crypto-analyzer.js.map