...
exports.CHECK_REGISTRY = [
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (evidenceType === 'dynamic-protocol') {
                ...
                if (!calibrationOk)
                    calibrationNote = ' incomplete-static-template';
            }
            const passed = baseOk && echDetected && calibrationOk;
            const missingParts = [];
            if (!networkCaps.hasTLS)
                missingParts.push('TLS');
            if (!networkCaps.hasQUIC)
                missingParts.push('QUIC');
            if (!networkCaps.hasHTX)
                missingParts.push('HTX');
            if (!networkCaps.port443)
                missingParts.push('port 443');
            if (!echDetected)
                missingParts.push('ECH');
            if (!calibrationOk)
                missingParts.push('calibration');
            let details;
            if (passed) {
                ...
                details = `❌ Missing: ${(0, format_1.missingList)(missingParts)}${calibrationNote}`;
            }
            return {
                id: 1,
                name: 'HTX over TCP-443 & QUIC-443',
    ...
    },
    {
        ...
        evaluate: async (analyzer) => {
            ...
            await analyzer.getStaticPatterns?.();
            const negative = analyzer.evidence?.negative;
            ...
            return {
                ...
                description: 'Requires ≥2 SCION path diversity, path management or IP-transition header, and negative assertion for legacy transition header',
                passed,
                details: passed ? `✅ SCION bridging, path diversity=${scionSupport.pathDiversityCount}, negative assertion enforced` : `❌ Missing: ${(0, format_1.missingList)([
    ...
    },
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (te && Array.isArray(te.endpoints)) {
                ...
                const paths = te.endpoints.map((e) => e.path).filter(Boolean);
                ...
                details = passed ? `✅ endpoints 1.1.0 present${legacyHTX || legacyQUIC ? ' + legacy 1.0.0' : ''}` : `❌ Missing: ${(0, format_1.missingList)([
    ...
    },
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (mix && typeof mix === 'object') {
                ...
                if (mix.uniquenessRatio !== undefined)
                    detailParts.push(`uniqueness=${(mix.uniquenessRatio * 100).toFixed(1)}%`);
                if (mix.diversityIndex !== undefined)
                    detailParts.push(`divIdx=${(mix.diversityIndex * 100).toFixed(1)}%`);
            }
    ...
    },
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (mix && typeof mix === 'object') {
                ...
                const pathLengths = mix.pathLengths || [];
...
exports.STEP_10_CHECKS = [
    ...
    },
    {
        ...
        evaluate: async (analyzer) => {
            ...
            const ev = analyzer.evidence || {};
            const neg = ev.negative;
            const forbiddenPresent = neg?.forbiddenPresent || [];
...
exports.PHASE_4_CHECKS = [
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (rl) {
                ...
                const capacities = buckets.map((b) => b.capacity).filter((n) => typeof n === 'number');
                const refills = buckets.map((b) => b.refillPerSec).filter((n) => typeof n === 'number');
                ...
                details = passed ? `✅ buckets=${bucketCount} global+scoped present capSpreadOk=${capacitySpreadOk} refillSpreadOk=${refillSpreadOk}` :
                    `❌ Rate-limit issues: ${(0, format_1.missingList)([
    ...
    // Phase 7: Statistical jitter variance enforcement
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (!sj)
                return { id: 26, name: 'Padding Jitter Variance', description: 'Enforces jitter stddev and mean bounds for adaptive padding distributions', passed: false, details: '❌ No jitter variance evidence', severity: 'minor', evidenceType: 'heuristic' };
            ...
            const details = passed ? `✅ jitter mean=${Math.round(mean)}ms std=${Math.round(std)}ms samples=${sj.samples || sj.sampleCount}` : `❌ Jitter variance issues: ${(0, format_1.missingList)([!sampleOk && 'insufficient samples', !meanOk && 'mean out of range', !stdOk && 'stddev out of range'])}`;
...
exports.PHASE_7_CONT_CHECKS = [
    {
        ...
        evaluate: async (analyzer) => {
            ...
            const mix = ev.mix;
            if (!mix)
                return { id: 27, name: 'Mix Advanced Variance', description: 'Validates entropy (≥4 bits), path length stddev (>0), and uniqueness/diversity thresholds', passed: false, details: '❌ No mix variance evidence', severity: 'minor', evidenceType: 'heuristic' };
    ...
    },
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (!vc)
                return { id: 29, name: 'Voucher/FROST Struct Validation', description: 'Validates voucher cryptographic struct base64 components & FROST threshold hints', passed: false, details: '❌ No voucherCrypto evidence', severity: 'minor', evidenceType: 'heuristic' };
            ...
            const details = passed ? `✅ struct base64 sizes ok n=${vc.frostThreshold?.n} t=${vc.frostThreshold?.t}` : `❌ Voucher struct issues: ${(0, format_1.missingList)([
    ...
    },
    {
        ...
        evaluate: async (analyzer) => {
            ...
            if (!at)
                return { id: 30, name: 'Access Ticket Rotation Policy', description: 'Validates structural access ticket evidence (fields, hex IDs) & rotation token presence', passed: false, details: '❌ No accessTicket evidence', severity: 'minor', evidenceType: 'heuristic' };
            const fieldsOk = Array.isArray(at.fieldsPresent) && at.fieldsPresent.includes('ticket') && at.fieldsPresent.includes('nonce') && at.fieldsPresent.includes('exp') && at.fieldsPresent.includes('sig');
            ...
            const details = passed ? `✅ accessTicket fields=${at.fieldsPresent.length} padVar=${at.paddingVariety} rateLimit=${rateLimitOk} conf=${at.structConfidence}${atDyn ? ` dynPadVar=${atDyn.uniquePadding} rotInt=${atDyn.rotationIntervalSec}s` : ''}` : `❌ Access ticket issues: ${(0, format_1.missingList)([
                !fieldsOk && 'core fields',
                !hexOk && 'hex IDs',
                !rotationOk && 'rotation token',
                !paddingOk && 'padding variety (≥2)',
...
exports.ALL_CHECKS = [...exports.CHECK_REGISTRY, ...exports.STEP_10_CHECKS, ...exports.PHASE_4_CHECKS, ...exports.PHASE_7_CONT_CHECKS];
...
for (const c of exports.STEP_10_CHECKS) {
    if (!exports.CHECK_REGISTRY.find(existing => existing.id === c.id))
        exports.CHECK_REGISTRY.push(c);
}
for (const c of exports.PHASE_4_CHECKS) {
    if (!exports.CHECK_REGISTRY.find(existing => existing.id === c.id))
        exports.CHECK_REGISTRY.push(c);
}
for (const c of exports.PHASE_7_CONT_CHECKS) {
    if (!exports.CHECK_REGISTRY.find(existing => existing.id === c.id))
        exports.CHECK_REGISTRY.push(c);
}
function getChecksByIds(ids) {
    const set = new Set(ids);
    return exports.ALL_CHECKS.filter(c => set.has(c.id)).sort((a, b) => a.id - b.id);
}
