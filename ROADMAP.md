...
4. [x] SCION Control Stream & Path Failover Metrics
  - Implemented: CBOR control stream evidence schema extended (offers{path,latencyMs,ts,flowId}, rawCborB64, schemaValid, signatureValid, pathSwitchLatenciesMs, probeIntervalsMs, avgProbeIntervalMs, maxPathSwitchLatencyMs, rateBackoffOk, timestampSkewOk, duplicateOfferDetected, duplicateWindowSec). Check 33 now enforces: ≥3 offers, ≥3 unique paths, no legacy legacy header, no duplicate offers, presence of latency metrics (NO_LATENCY_METRICS), max path switch latency ≤300ms (PATH_SWITCH_LATENCY_HIGH), probe interval list & average within 50–5000ms (PROBE_INTERVAL_OUT_OF_RANGE / NO_PROBE_INTERVALS), explicit backoff flag (BACKOFF_VIOLATION/BACKOFF_UNKNOWN), timestamp skew flag (TS_SKEW/TS_SKEW_UNKNOWN), control stream signature indicators (SIGNATURE_INVALID/SIGNATURE_MISSING), schema validation (SCHEMA_INVALID/SCHEMA_UNKNOWN), plus existing structural failures (INSUFFICIENT_OFFERS, INSUFFICIENT_UNIQUE_PATHS, LEGACY_HEADER_PRESENT, DUPLICATE_OFFER, CBOR_PARSE_ERROR). Tests added covering positive pass and each new failure code category (latency high, probe interval invalid, backoff violation, TS skew, signature invalid, schema invalid) alongside existing negatives. Future (non-blocking): real cryptographic signature verification over control stream, precise token bucket quantitative thresholds, rolling duplicate detection with configurable window, deeper CBOR schema (currently boolean flag), per-offer expiration & dynamic harness live timing capture.
5. [ ] Bootstrap PoW & Multi-Bucket Rate-Limit Statistics
  ...
  - Caveats (non-blocking): constant-time PoW verify (future), larger configurable rolling windows, percentile band parameterization, deeper statistical justification for thresholds.
